...
class Runcontrol:
    ...
    agg_method: Optional[str] = "Ensemble"      # The Aggreagation Method
    ...
    plotsavefig: bool = True                    # save figures
...
@dataclass
class Softmax:
    optimizer_type: Any = "rmsprop"                # str or List[str] the optimizer type: adam, sgd, rmsprop
    optimizer_param: Any = None                    # None|dict|List[dict] optimizer parameters
    initializer: Any = "glorot_uniform"            # str or List[str] the initializer: glorot_uniform, he_normal, random_normal
    regularization: Any = None                     # None|"l1"|"l2"|"hybrid" or List, l1 or l2 or hybrid regularization
    regularization_rate: Any = 0.01                # float or List[float] (for 'hybrid', user may pass [l1,l2]), regularization rate. should be a list of length 2 if regularization is hybrid
    batch_size: Any = 32                           # int or List[int], batch size
    epochs: Any = 1000                             # int or List[int], number of epochs
    patience: Any = 10                             # int or List[int], patience epochs for earlystopping
    buffer_size: Any = None                        # None|int or List, buffer size for shuffling inside softmax
    shuffle: Any = True                            # bool or List, shuffle the training data in every epoch
    verbose: Any = 0                               # int or List[int], frequency of displaying softmax training results
...
class User:
    ...
    numClass: Optional[int] = None                  # number of class labels in ytrain, yval, and ytest 
...
class ClassifierConfig:
    ...
    softmax: Softmax = field(default_factory=Softmax)
    ...
    @classmethod
    def from_legacy(cls, params: Dict[str, Any]) -> "ClassifierConfig":
        ...
        run = Runcontrol(
            ...
            agg_method=g("runcontrol_agg_method", "Ensemble"),
        ...
        smax = Softmax(
            optimizer_type=g("softmax_optimizer_type", "rmsprop"),
            optimizer_param=g("softmax_optimizer_param", None),
            initializer=g("softmax_initializer", "glorot_uniform"),
            regularization=g("softmax_regularization", None),
            regularization_rate=g("softmax_regularization_rate", 0.01),
            batch_size=g("softmax_batch_size", 32),
            epochs=g("softmax_epochs", 1000),
            patience=g("softmax_patience", 10),
            buffer_size=g("softmax_buffer_size", None),
            shuffle=g("softmax_shuffle", True),
            verbose=g("softmax_verbose", 0),
        )
        user = User(
            ...
            numClass=g("userdata_numClass", None),
        ...
        return cls(runcontrol=run, selection=sel, nodes=nodes, operator=op,
                   gene=gene, tree=tree, fitness=fit, softmax=smax,
