...
from .gp_config_regress import gp_config

class gpregressor:
    def __init__(self, cfg: RegressorConfig | None = None, **legacy_params):
        ...
        if cfg is None:
            if had_legacy:
                # Requires RegressorConfig.from_legacy(...) in your dataclass module
                cfg = RegressorConfig.from_legacy(legacy_params)
            else:
                cfg = RegressorConfig()

        self.cfg: RegressorConfig = cfg
        ...
        if had_cfg and had_legacy:
            self.error("Provide either cfg=RegressorConfig(...) OR legacy keyword arguments, not both.", TypeError)
    ...
    def _init_logging(self) -> None:
        ...
        self.logger = get_logger(f"genforge.gpregressor.{id(self)}")
    ...
    def configure(self) -> None:
        ...
        if agg == "regression":
            from .gp_individuals_ensemble_regress import gp_individuals_ensemble
            from .gp_state_init_ensemble_regress import gp_state_init_ensemble
            from .gp_fitness_init_regress import gp_fitness_init
            gp_individuals_ensemble(self)
            gp_state_init_ensemble(self)
            gp_fitness_init(self)
        else:
            self.warning(f"Unknown agg_method '{agg}', using regression wiring.")

    @log_exceptions()
    def clearcache(self) -> None:
        if self.config['runcontrol']['usecache']:
            from .gp_cache_regress import gp_cache
            gp_cache(self)

    @log_exceptions()
    def track_param(self) -> None:
        if self.config['runcontrol']['agg_method'].lower() == 'regression':
            from .gp_track_param_ensemble_regress import gp_track_param_ensemble
            gp_track_param_ensemble(self)

    @log_exceptions()
    def build_pop(self) -> None:
        from .gp_popbuild_init import gp_popbuild_init
        from .gp_popbuild_regress import gp_popbuild
    ...
    def evalfitness(self) -> None:
        if self.config['runcontrol']['parallel']['useparallel']:
            from .gp_evalfitness_par_regress import gp_evalfitness_par
            gp_evalfitness_par(self)
        else:
            from .gp_evalfitness_ord_regress import gp_evalfitness_ord
            gp_evalfitness_ord(self)

    @log_exceptions()
    def updatestats(self) -> None:
        from .gp_updatestats_regress import gp_updatestats
        gp_updatestats(self)

    @log_exceptions()
    def displaystats(self) -> None:
        from .gp_displaystats_regress import gp_displaystats
    ...
    def plotstats(self) -> None:
        ...
        from .gp_plotfitness_regress import gp_plotfitness
        from .gp_plotrankall_regress import gp_plotrankall
        from .gp_plotrankbest_regress import gp_plotrankbest
    ...
    @log_exceptions()
    def adaptiveinject(self):
        """Adaptive Orthogonal Symbolic Injection Module"""
        from .gp_adaptiveinject import gp_adaptiveinject
        # from .gp_adaptiveinject_all import gp_adaptiveinject
        gp_adaptiveinject(self)

    @log_exceptions()
    def postprocess(self):
        """Replace Z with the expressions"""
        from .gp_postprocess import gp_postprocess
        gp_postprocess(self)

    @log_exceptions()
    def report(self, ensemble_row: int = 0, out_path: str = None, title: str = None) -> str:
        """
        Generate an HTML report for the chosen ensemble.

        Saves to ./<gp.userdata['name']>_ensemble_<ensemble_row>.html by default.
        Returns the absolute path to the written file.
        """
        # Local imports avoid adding global deps unless used
        from .gp_html_report_regress import write_ensemble_html
        import os, re

        # Human title
        run_name = self.userdata.get('name')
        if title is None:
            title = f"{run_name}: chosen ensemble #{ensemble_row}"

        # Safe default filename in the current folder
    ...
    @log_exceptions()
    def predict(self, X, *, ensemble_row: int = 0,
            mode: str = "ensemble", id_pop: int | None = None, id_ind: int | None = None):
        from .gp_predict_regress import gp_predict
        """
        ...
        np.ndarray
            predicted values
        """
        return gp_predict(self, X, ensemble_row=ensemble_row, mode=mode, id_pop=id_pop, id_ind=id_ind)
    ...
    def plotcomplexity(self, mode: str = "all", id_pop: int | None = None,
        ...
        from .gp_plotcomplexity_regress import gp_plotcomplexity
