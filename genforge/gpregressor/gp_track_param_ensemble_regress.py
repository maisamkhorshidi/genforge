...
def gp_track_param_ensemble(gp):
    ...
    if gen == 0:
        ...
        gp.track = {
            ...
            'ensemble_weight': [np.full((pop_size, num_pop + 1), np.inf)],
            'yp': {
                'isolated': {
            ...
            'rank': {
                ...
                'objective': {
                    'isolated': [[np.full((pop_size), np.inf) for _ in range(num_pop)]],
                    'ensemble': [np.full((pop_size), np.inf)],
                    },
                },
            'all_ensemble':{
        ...
        gp.track['ensemble_weight'].append(np.full((pop_size, num_pop + 1), np.inf))
        ...
        gp.track['penalty']['isolated']['train'].append(np.zeros((pop_size, num_pop)))
        gp.track['penalty']['isolated']['validation'].append(np.zeros((pop_size, num_pop)))
        gp.track['penalty']['isolated']['test'].append(np.zeros((pop_size, num_pop)))
        gp.track['penalty']['ensemble']['train'].append([np.zeros((pop_size))]),
        gp.track['penalty']['ensemble']['validation'].append(np.zeros((pop_size)))
        gp.track['penalty']['ensemble']['test'].append(np.zeros((pop_size)))
        gp.track['objective']['isolated'].append(np.full((pop_size, num_pop), np.inf))
        gp.track['objective']['ensemble'].append(np.full((pop_size), np.inf))
        ...
        gp.track['rank']['objective']['isolated'].append([np.full((pop_size), np.inf) for _ in range(num_pop)])
        gp.track['rank']['objective']['ensemble'].append(np.full((pop_size), np.inf))
